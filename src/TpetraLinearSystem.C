...
void
TpetraLinearSystem::segregateProblem(Teuchos::RCP<LinSys::Matrix> inputMatrix,
...
{
  // Step 1: do some checks to make sure we can segregate the matrix properly
  const Tpetra::global_size_t globalNumRows    = inputMatrix->getGlobalNumRows();
  const size_t                localNumRows     = inputMatrix->getNodeNumRows();
  const size_t                localNumCols     = inputMatrix->getNodeNumCols();
  const Tpetra::global_size_t globalNumEntries = inputMatrix->getGlobalNumEntries();
  const size_t                localNumEntries  = inputMatrix->getNodeNumEntries();

  if(globalNumRows % numDof_ > 0) {std::cout << "segregateMatrix: globalNumRows in input matrix is not divisible by numDofs_!" << std::endl;}
  if(localNumRows  % numDof_ > 0) {std::cout << "segregateMatrix: localNumRows in input matrix is not divisible by numDofs_!" << std::endl;}
  if(globalNumEntries % (numDof_*numDof_) > 0) {std::cout << "segregateMatrix: globalNumEntries in input matrix is not divisible by numDofs_*numDofs_!" << std::endl;}
  if(localNumEntries  % (numDof_*numDof_) > 0) {std::cout << "segregateMatrix: localNumEntries in input matrix is not divisible by numDofs_*numDofs_!" << std::endl;}

  const size_t segregatedLocalNumRows    = localNumRows  / numDof_;
  const size_t segregatedLocalNumEntries = localNumEntries / (numDof_*numDof_);

  // Step 2: create some segregated maps
  Teuchos::RCP<LinSys::Map> segregatedRowMap;
  segregateMap(inputMatrix->getCrsGraph()->getRowMap(), segregatedRowMap);
  Teuchos::RCP<LinSys::Map> segregatedColMap;
  segregateMap(inputMatrix->getCrsGraph()->getColMap(), segregatedColMap);
  Teuchos::RCP<LinSys::Map> segregatedRangeMap;
  segregateMap(inputMatrix->getCrsGraph()->getRangeMap(), segregatedRangeMap);
  Teuchos::RCP<LinSys::Map> segregatedDomainMap;
  segregateMap(inputMatrix->getCrsGraph()->getDomainMap(), segregatedDomainMap);

  // Step 3: construct CrsMatrix, Rhs and Sln
  segregatedMatrix = Teuchos::rcp(new LinSys::Matrix(segregatedRowMap,
                                                     segregatedColMap,
                                                     0,
                                                     Tpetra::StaticProfile));
  segregatedRhs = Teuchos::rcp(new LinSys::MultiVector(segregatedRowMap, numDof_));
  segregatedSln = Teuchos::rcp(new LinSys::MultiVector(segregatedRowMap, numDof_));

  // Step 4: create the rowPtrs, colInds and values for segregated CrsMatrix
  Teuchos::ArrayRCP<const size_t> rowPtrs;
  Teuchos::ArrayRCP<const LinSys::Graph::local_ordinal_type> colInds;
  Teuchos::ArrayRCP<const LinSys::Matrix::scalar_type> values;
  inputMatrix->getAllValues(rowPtrs, colInds, values);
  Teuchos::ArrayRCP<const LinSys::Matrix::scalar_type> inputRhsData = inputRhs->getData();
  Teuchos::ArrayRCP<const LinSys::Matrix::scalar_type> inputSlnData = inputSln->getData();

  Teuchos::ArrayRCP<size_t> segregatedRowPtrs(segregatedLocalNumRows + 1);
  Teuchos::ArrayRCP<LinSys::Graph::local_ordinal_type> segregatedColInds(segregatedLocalNumEntries);
  Teuchos::ArrayRCP<LinSys::Matrix::scalar_type> segregatedValues(segregatedLocalNumEntries);
  Teuchos::Array<Teuchos::ArrayRCP<LinSys::Matrix::scalar_type> > segregatedRhsData(numDof_);
  Teuchos::Array<Teuchos::ArrayRCP<LinSys::Matrix::scalar_type> > segregatedSlnData(numDof_);
  for(size_t dof = 0; dof < numDof_; ++dof) {
    segregatedRhsData[dof] = segregatedRhs->getDataNonConst(dof);
    segregatedSlnData[dof] = segregatedSln->getDataNonConst(dof);
  }

  LinSys::Map::local_ordinal_type numEntriesInRow = 0;
  for(size_t rowIdx = 0; rowIdx < segregatedLocalNumRows; ++rowIdx) {
    numEntriesInRow = (rowPtrs[rowIdx*numDof_ + 1] - rowPtrs[rowIdx*numDof_]) / numDof_;

    // Load CrsMatrix data
    segregatedRowPtrs[rowIdx + 1] = segregatedRowPtrs[rowIdx] + numEntriesInRow;
    for(LinSys::Map::local_ordinal_type entryIdx = 0; entryIdx < numEntriesInRow; ++entryIdx) {
      segregatedColInds[segregatedRowPtrs[rowIdx] + entryIdx]
        = colInds[rowPtrs[rowIdx*numDof_] + entryIdx*numDof_] / numDof_;
      segregatedValues[segregatedRowPtrs[rowIdx] + entryIdx]
        = values[rowPtrs[rowIdx*numDof_] + entryIdx*numDof_];
      if( (colInds[rowPtrs[rowIdx*numDof_] + entryIdx*numDof_] / numDof_) >= (localNumCols / numDof_) ) {
        std::cout << "In row " << rowIdx << ", column index " << entryIdx << " has column id "
                  << colInds[rowPtrs[rowIdx*numDof_] + entryIdx*numDof_] / numDof_
                  << " larger than localNumCols: " << localNumCols / numDof_ << std::endl;
      }
    }

    // Load MultiVector data
    for(size_t dof = 0; dof < numDof_; ++dof) {
      segregatedRhsData[dof][rowIdx] = inputRhsData[rowIdx*numDof_ + dof];
      segregatedSlnData[dof][rowIdx] = inputSlnData[rowIdx*numDof_ + dof];
    }
  }

  // Step 5: construct and fillComplete segregated CrsMatrix
  segregatedMatrix->setAllValues(segregatedRowPtrs, segregatedColInds, segregatedValues);
  segregatedMatrix->expertStaticFillComplete(segregatedDomainMap, segregatedRangeMap);
}

void
TpetraLinearSystem::segregateMap(Teuchos::RCP<const LinSys::Map> inputMap,
                                 Teuchos::RCP<LinSys::Map>& segregatedMap)
{
  Teuchos::ArrayView<const typename LinSys::Map::global_ordinal_type> ownedRowGIDs = inputMap()->getNodeElementList();
  const typename LinSys::Map::local_ordinal_type numLocalRows = ownedRowGIDs.size() / numDof_;
  Teuchos::ArrayRCP<typename LinSys::Map::global_ordinal_type> segregatedOwnedRowGIDs(numLocalRows);
  for(typename LinSys::Map::local_ordinal_type idx = 0; idx < numLocalRows; ++idx) {
    segregatedOwnedRowGIDs[idx] = ownedRowGIDs[numDof_*idx];
  }
  segregatedMap = Teuchos::rcp(new LinSys::Map(inputMap->getGlobalNumElements() / numDof_,
                                               segregatedOwnedRowGIDs(),
